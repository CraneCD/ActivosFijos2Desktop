...
def render_spells_picker(class_index: Optional[str], subclass_index: Optional[str]) -> List[str]:
    ...
    if choice:
        ...
        st.markdown(f"**{detail.get('name','')}** — Level {detail.get('level', 0)} {', '.join(detail.get('components', []))}")
...
def main():
    st.set_page_config(page_title="D&D 5.5e Character Sheet", page_icon="🧙", layout="wide")
    ...
    with col_stats:
        ...
        cols = st.columns(3)
        with cols[0]:
            st.metric("AC (base)", 10 + max(0, ability_modifier(scores["Dexterity"])) )
    ...
    with col_b:
        st.download_button(
            ...
            data=st.session_state.get("char_json", "").encode("utf-8") if st.session_state.get("char_json") else str({
                "name": name,
                "alignment": alignment,
                "level": int(level),
                "race": race_name,
