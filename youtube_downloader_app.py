...
with st.sidebar:
    ...
    if has_ffmpeg:
        ...
        # Always show the install button on Windows when FFmpeg is not found
        if current_os == 'Windows':
            st.markdown("### 📥 Auto-Install")
            install_clicked = st.button("📥 Install FFmpeg (Windows)", type="primary", use_container_width=True, key="install_ffmpeg_btn")
            if install_clicked:
                import subprocess
            ...
            try:
                with st.spinner("Downloading..."):
                    ...
                    with tempfile.TemporaryDirectory() as tmpdir:
                        ...
                        if downloaded_files:
                            # If multiple files (video + audio), we have a problem - but this shouldn't happen without ffmpeg
